import torch
from utils import (dice_coef, ahd_coef, hausdorff_coef, hd95_coef,
                    nsd_coef, biou_coef, class2one_hot)

def make_disk(shape, center, radius):
    yy, xx = torch.meshgrid(torch.arange(shape[0]), torch.arange(shape[1]), indexing='ij')
...
for name, pred in [("perfect", pred_perfect), ("shifted_3px", pred_shifted), ("smaller_radius", pred_small)]:
